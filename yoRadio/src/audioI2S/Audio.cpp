...
#include "micro_flac/flac_decoder.h" // single include path (flat vendor layout)
...
// Runtime microflac debug switch (UART/telnet command-controlled).
// Default follows build-time OMNIA_DEBUG_MICROFLAC (DEBUG builds start verbose).
volatile bool g_omnia_microflac_debug = (OMNIA_DEBUG_MICROFLAC != 0);
...
static const char* mfErrName(int e){
    switch(e){
        case -1:  return "BAD_HEADER";
        case -2:  return "SYNC_NOT_FOUND";
        case -3:  return "CRC_MISMATCH";
        case -4:  return "MEMORY_ALLOCATION";
        case -5:  return "BAD_MAGIC_NUMBER";
        case -14: return "OGG_DEMUX";
        case -15: return "OGG_BAD_HEADER";
        case -16: return "INPUT_INVALID";
        case -17: return "OUTPUT_TOO_SMALL";
        case -18: return "FRAME_MISMATCH";
        case -19: return "INTERNAL";
        case -21: return "INVALID_ARGUMENT";
        default:  return "ERR";
    }
}
...
void AudioBuffer::changeMaxBlockSize(uint16_t mbs) {
    // OMNIA Log59 (R5): residual tail must be >= maxBlockSize — getReadPtr() wrap does
    // memcpy(m_endPtr, m_buffer, maxBlockSize - len) into the residual area. A block size
    // above m_resBuffSize would overflow the reserve and silently corrupt decoder state.
    if((size_t)mbs > m_resBuffSize) {
        if(!m_f_blockClampLogged) {
            m_f_blockClampLogged = true;
            AUDIO_ERROR("AudioBuffer: maxBlockSize %u > residual %u -> clamped", (unsigned)mbs, (unsigned)m_resBuffSize);
        }
        mbs = (uint16_t)m_resBuffSize;
    }
    m_maxBlockSize = mbs;
    return;
}
...
m_i2s_chan_cfg.role = I2S_ROLE_SLAVE;      // STM32 is master now
...
m_i2s_std_cfg.clk_cfg = I2S_STD_CLK_DEFAULT_CONFIG(48000);
m_i2s_std_cfg.clk_cfg.mclk_multiple = I2S_MCLK_MULTIPLE_256;
// если это поле есть в твоём core/IDF — оставить:
// m_i2s_std_cfg.clk_cfg.bclk_div = 8;
...
//    m_fileStartTime = fileStartPos;
    ...
    if(c_path.ends_with_icase(".m4a")) { codec = CODEC_M4A; AUDIO_INFO("format is m4a"); }
...
uint32_t Audio::stopSong() {
    ...
    m_f_lockInBuffer = true; // wait for the decoding to finish
        ...
        // Chat16 Step0: free index on STOP / track change to avoid leak
        omnia_m4aIndexReset();
        omnia_aacIndexReset();
...
//            log_w("%lu, lastGranulePosition %llu", __LINE__, m_lastGranulePosition);
        ...
        if(m_f_stream && m_codec == CODEC_FLAC && (m_dataMode == AUDIO_DATA || m_streamType == ST_WEBFILE) && !m_mf_active){
            ...
            m_mf_active = true;
                    ...
                    InBuff.changeMaxBlockSize(MF_WEB_BLOCK_DEFAULT); // clamped to residual inside (Log59 R5)
                    ...
                    AUDIO_INFO("microflac WEB activated: block=%u resBuff=%u feedCap=%u", (unsigned)InBuff.getMaxBlockSize(), (unsigned)InBuff.getResBuffSize(), (unsigned)m_mf_feedCap);
    ...
    switch(m_codec) {
        ...
        case CODEC_FLAC: {
            ...
            if(isWeb){
                ...
                if(m_mf_lastProgressMs == 0) m_mf_lastProgressMs = millis();

                // honest block target: never above the InBuff residual reserve (Log59 R5)
                uint16_t mfBlockTarget = MF_WEB_BLOCK_DEFAULT;
                if((size_t)mfBlockTarget > InBuff.getResBuffSize()) mfBlockTarget = (uint16_t)InBuff.getResBuffSize();
                if(InBuff.getMaxBlockSize() < mfBlockTarget){
                    InBuff.changeMaxBlockSize(mfBlockTarget);
                    m_mf_webBlock = InBuff.getMaxBlockSize();
                }
                if(m_mf_feedCap < m_mf_webBlock) m_mf_feedCap = m_mf_webBlock; // keep field consistent with block
                ...
                // --- flagship local decode loop: keep decoding while input yields progress ---
                ...
                while(loops++ < MF_WEB_MAX_LOOPS && left > 0){
                    ...
                    int32_t* out = (m_mf_header_ready && m_mf_out32 && m_mf_out32_cap) ? m_mf_out32 : nullptr;
...
if (g_omnia_microflac_debug) {
                    ...
                    if(mf_res == micro_flac::FLAC_DECODER_HEADER_READY){
                        ...
                        m_mf_bits = info.bits_per_sample(); // 16 or 24 OK (24 is downscaled to 16 on drain)
                        ...
                        if(!m_mf_streamLogged){
                            m_mf_streamLogged = true;
                            AUDIO_INFO("microflac stream: max_block=%u min_block=%u",
                                       (unsigned)info.max_block_size(), (unsigned)info.min_block_size());
                        }

                        size_t needed = g_mf.get_output_buffer_size_samples(); // interleaved samples = max_block_size * channels
                        size_t mfFloor = (size_t)m_mf_channels * 4096;
                        if(needed < mfFloor) needed = mfFloor; // safety floor
                        if(needed > (size_t)m_mf_channels * 65535){
                            AUDIO_ERROR("microflac: absurd output size (%u samples) -> stop", (unsigned)needed);
                            stopSong();
                            return -1;
                        }
                        if(!m_mf_out32 || m_mf_out32_cap < needed){ // keep existing out32 if already large enough
                            free(m_mf_out32);
                            m_mf_out32 = (int32_t*) ps_malloc(needed*sizeof(int32_t));
                            if(!m_mf_out32){
                        ...
                        m_mf_prevErr = 0;
                        m_mf_errSoftCnt = 0;
                        AUDIO_INFO("microflac HEADER_READY sr=%lu ch=%u bps=%u max_blk=%u out_cap=%u consumed=%u left=%u",
                                   (unsigned long)m_mf_sample_rate, m_mf_channels, m_mf_bits,
                                   (unsigned)info.max_block_size(), (unsigned)m_mf_out32_cap,
                                   (unsigned)bytes_consumed, (unsigned)left);
                        // IMPORTANT: continue loop immediately to decode first audio frames
                        continue;
                    }

                    if(mf_res == micro_flac::FLAC_DECODER_SUCCESS){
                    ...
                    if(mf_res == micro_flac::FLAC_DECODER_NEED_MORE_DATA){
                        if(bytes_consumed == 0){
                            m_mf_zeroConsumeCnt++;
...
if (g_omnia_microflac_debug) {
                                ...
                                AUDIO_INFO("microflac OGG_DBG state=%d assembling=%u skipping=%u pkt=%u bodyCons=%u seg=%u/%u bufCap=%u bufPeak=%u",
                                    (int)g_mf.get_last_ogg_dbg_state(),
                                    (unsigned)g_mf.get_last_ogg_dbg_assembling(),
                                    (unsigned)g_mf.get_last_ogg_dbg_skipping(),
                                    (unsigned)g_mf.get_last_ogg_dbg_packet_size(),
                    ...
                    if(mf_res == micro_flac::FLAC_DECODER_END_OF_STREAM){
                        m_f_eof = true;
                        break;
                    }
                    ...
                    if(!m_mf_header_ready){
                        if(m_mf_headerReadyAtMs){
                            // re-lock after a mid-stream decoder reset failed (BOS headers only come
                            // with a fresh connection) -> reconnect, do not stopSong
                            AUDIO_ERROR("microflac re-lock failed -> reconnect");
                            stallReconnect("mf_relock");
                            return 0; // CRITICAL: never continue after reconnect
                        }
                        // fatal before the very first header -> stop cleanly via decodeError(-100)
                        AUDIO_ERROR("microflac fatal before header ready -> stop");
                        microflac_reset_();
                        m_mf_active = true; // keep micro-flac mode for next connect
                        fatal_before_header = true;
                        break;
                    }

                    // AFTER header:
                    if(mf_res == micro_flac::FLAC_DECODER_ERROR_OUTPUT_TOO_SMALL){
                        size_t needed = g_mf.get_output_buffer_size_samples();
                        if(needed > m_mf_out32_cap){
                            free(m_mf_out32);
                            m_mf_out32 = (int32_t*) ps_malloc(needed*sizeof(int32_t));
                            if(!m_mf_out32){
                                AUDIO_ERROR("microflac: out32 realloc failed (%u samples)", (unsigned)needed);
                                m_mf_out32_cap = 0;
                                stopSong();
                                return -1;
                            }
                            m_mf_out32_cap = needed;
                            AUDIO_INFO("microflac out32 realloc -> %u samples", (unsigned)needed);
                        }
                        continue;
                    }
                    ...
                    if(bytes_consumed == 0 && (int8_t)mf_res == m_mf_prevErr &&
                       ((int)mf_res == micro_flac::FLAC_DECODER_ERROR_OGG_DEMUX ||
                        ...
                        g_mf.reset();
                        // re-apply radio config lost by reset (see microflac_reset_)
                        g_mf.set_crc_check_enabled(false);
                        g_mf.set_max_metadata_size(micro_flac::FLAC_METADATA_TYPE_PICTURE, 0);
                        g_mf.set_max_metadata_size(micro_flac::FLAC_METADATA_TYPE_VORBIS_COMMENT, 0);
                        m_mf_header_ready = false;   // wait for HEADER_READY again
                        m_mf_pending_total = 0;
                        m_mf_pending_off = 0;
                        m_mf_prevErr = 0;
                        m_mf_waitHeaderSinceMs = millis();
                        // KEEP out32 (size is re-validated on next HEADER_READY)
                        // resync input to next OggS
                        if(left > 0){
                            size_t k = 1;
                            for(; k + 3 < left; k++){
                                if(p[k]=='O' && p[k+1]=='g' && p[k+2]=='g' && p[k+3]=='S') break;
                            }
                            if(k + 3 >= left) k = (left > 3) ? (left - 3) : left; // keep possible page-mark tail
                ...
                // set leftovers for common epilogue (common code does bytesDecoded = len - bytesLeft)
                ...
                // ZERO-CONSUME reconnect policy: time-based, with grace window after HEADER_READY.
                // NEVER reconnect solely because zero_consume hit the counter quickly after header.
                {
...
//            info(evt_lyrics, "%s", m_syltLines[m_cat.syltIdx].c_get());
    ...
    // Chat16 Step5: flagship scale - override time by index if available (more accurate than bitrate for VBR)
    if(m_dataMode==AUDIO_LOCALFILE){
        uint32_t curFilePos = (m_audioFilePosition > inBufferFilled()) ? (m_audioFilePosition - inBufferFilled()) : 0;
        ...
        if(m_codec==CODEC_M4A && m_m4aSecOff && m_m4aDurSec>0 && m_m4aBuildNextSec>1){
            ...
            if(sec <= m_m4aDurSec){
                // Optional: interpolate inside second using frame count? For now use sec
                // More accurate: we could compute fraction via (offset - secOff[sec]) / (secOff[sec+1]-secOff[sec])
                m_audioCurrentTime = sec;
                // If we have next sec, interpolate
                if(sec+1 < m_m4aBuildNextSec && m_m4aSecOff[sec+1] > m_m4aSecOff[sec]){
                    uint32_t secSize = m_m4aSecOff[sec+1] - m_m4aSecOff[sec];
                    uint32_t inside = offset - m_m4aSecOff[sec];
                    float frac = secSize ? (float)inside / secSize : 0;
                    m_audioCurrentTime = sec + (uint32_t)(frac); // keep integer sec for now, could add frac*1 sec
                    // For more precise, add frac
                    audioCurrentTime = sec + frac;
                    m_audioCurrentTime = (uint32_t)audioCurrentTime;
                }
            }
        }
...
// ===== OMNIA M4A seek index implementation (flagship) =====
...
void Audio::omnia_m4aIndexInitIfPossible(){
  ...
  if(!m_localFS || m_localPath.length()==0) return;
  if(m_stsz_position==0 || m_stsz_numEntries==0) return;
  omnia_m4aIndexReset();
  uint32_t sr = m_M4A_sampleRate ? m_M4A_sampleRate : (getSampleRate()?getSampleRate():44100);
  if(sr==0) sr=44100;
  if(m_audioFileDuration==0){
    uint64_t durMs = (uint64_t)m_stsz_numEntries * 1024ULL * 1000ULL / sr;
    m_audioFileDuration = (durMs + 500) / 1000;
  }
  ...
  if(psramFound()){
    ...
    m_m4aSecFrame = (uint32_t*) ps_malloc(n * sizeof(uint32_t));
  } else {
    m_m4aSecOff = (uint32_t*) malloc(n * sizeof(uint32_t));
    m_m4aSecFrame = (uint32_t*) malloc(n * sizeof(uint32_t));
  }
  ...
  m_m4aSecFrame[0] = 0;
  m_m4aIdxValid = true;
  AUDIO_INFO("omnia_m4aIndexInit dur=%lus %lums stszPos=%lu entries=%lu secArray=%lu PSRAM=%d", (unsigned long)m_m4aDurSec, (unsigned long)m_m4aDurMs, (unsigned long)m_stsz_position, (unsigned long)m_stsz_numEntries, (unsigned long)n, psramFound());
}

static inline uint32_t be32_m4a(const uint8_t* b){
...
void Audio::omnia_m4aIndexTick(uint16_t maxEntries){
  ...
  for(uint16_t i=0; i<maxEntries && m_m4aBuildNextFrame < m_m4aFramesTotal; i++){
    ...
    while(m_m4aBuildNextSec <= m_m4aDurSec){
      uint32_t sr = m_M4A_sampleRate ? m_M4A_sampleRate : (getSampleRate()?getSampleRate():44100);
      uint32_t frameBoundary = (uint64_t)m_m4aBuildNextSec * sr / 1024ULL;
      if(m_m4aBuildNextFrame < frameBoundary) break;
      m_m4aSecFrame[m_m4aBuildNextSec] = m_m4aBuildNextFrame;
      m_m4aSecOff[m_m4aBuildNextSec] = (uint32_t)m_m4aBuildCumBytes;
      m_m4aBuildNextSec++;
    }
  }
}
...
bool Audio::omnia_m4aSeekMs(uint32_t ms){
  ...
  const uint32_t samplesPerFrame = 1024;
  ...
  // Fast path via per-sec index if built
  if(m_m4aSecFrame && m_m4aSecOff && m_m4aDurSec>0){
    uint32_t sec = ms / 1000;
    if(sec > m_m4aDurSec) sec = m_m4aDurSec;
    if(sec < m_m4aBuildNextSec){
      uint32_t baseOff = m_m4aSecOff[sec];
      uint32_t baseFrame = m_m4aSecFrame[sec];
      if(targetFrame >= baseFrame){
        uint32_t framesInside = (uint32_t)(targetFrame - baseFrame);
        ...
        if(framesInside>0){
          ...
          if(fSeek){
            ...
            while(remaining>0){
              ...
              for(uint16_t i=0;i<chunk;i++) sumInside += be32_m4a(buf+i*4);
              remaining -= chunk;
            }
          } else {
            if(audioFileSeek(stszBase)==0){
              uint8_t buf[64*4];
              uint32_t remaining = framesInside;
              while(remaining>0){
                uint16_t chunk = remaining>64?64:remaining;
                ...
                uint16_t got=0;
                while(got < bytes){
                  int32_t r = audioFileRead(buf+got, bytes-got);
                  if(r<=0){ vTaskDelay(1); continue; }
                  got+=r;
                }
                for(uint16_t i=0;i<chunk;i++) sumInside += be32_m4a(buf+i*4);
        ...
        uint32_t pos = audioStart + baseOff + (uint32_t)sumInside;
        if(pos >= endPos) pos = endPos - 1;
        m_m4aSeekExactNext = true;
        AUDIO_INFO("m4aSeekMs FAST sec=%lu baseFrame=%lu baseOff=%lu inside=%lu -> pos %lu (target %llu)", (unsigned long)sec, (unsigned long)baseFrame, (unsigned long)baseOff, (unsigned long)framesInside, (unsigned long)pos, (unsigned long long)targetFrame);
        return setFilePos(pos);
      }
    }
  }

  // Fallback: no index or sec not built yet
  ...
  // Accurate O(N) fallback when index not ready
  uint64_t cumBytes = 0;
  uint32_t framesToSum = (uint32_t)targetFrame;
  if(framesToSum>0){
    ...
    while(remaining>0){
      ...
      uint16_t got=0;
      while(got < bytes){
        int32_t rd = audioFileRead(buf+got, bytes-got);
        if(rd<=0){ vTaskDelay(2); continue; }
        got+=rd;
      }
      for(uint16_t i=0;i<toRead;i++) cumBytes += be32_m4a(buf+i*4);
      remaining -= toRead;
    }
  }
  ...
  if(pos >= endPos) pos = endPos - 1;
  m_m4aSeekExactNext = true;
  AUDIO_INFO("m4aSeekMs accurate ms=%lu sr=%lu frame=%llu cum=%llu -> pos %lu (stsz %lu) [no sec index yet]", (unsigned long)ms, (unsigned long)sampleRate, (unsigned long long)targetFrame, (unsigned long long)cumBytes, (unsigned long)pos, (unsigned long)m_stsz_numEntries);
  return setFilePos(pos);
}
...
static const uint32_t adtsSampleRates[16] = {96000,88200,64000,48000,44100,32000,24000,22050,16000,12000,11025,8000,7350,0,0,0};

static inline bool adtsParseHeader(const uint8_t *hdr, uint32_t &frameLen, uint32_t &sampleRate, uint32_t &samples){
  // Check sync 0xFFF
  if(hdr[0]!=0xFF || (hdr[1] & 0xF0)!=0xF0) return false;
  uint8_t sfIdx = (hdr[2] & 0x3C) >> 2;
  if(sfIdx >= 13) return false;
  sampleRate = adtsSampleRates[sfIdx];
  if(sampleRate==0) return false;
  uint8_t protection = hdr[1] & 0x01;
  uint8_t hdrLen = protection ? 7 : 9;
  frameLen = ((hdr[3] & 0x03) << 11) | (hdr[4] << 3) | ((hdr[5] & 0xE0) >> 5);
  if(frameLen < hdrLen) return false;
  uint8_t numBlocks = (hdr[6] & 0x03);
  uint32_t blocks = numBlocks + 1;
  samples = 1024 * blocks;
  return true;
}

void Audio::omnia_aacIndexInitIfPossible(){
  if(m_codec != CODEC_AAC) return;
  if(!m_localFS || m_localPath.length()==0) return;
  if(m_audioFileSize==0) return;
  omnia_aacIndexReset();
  ...
  // Use fileSize / (128k/8) = fileSize /16000 approx sec, plus margin. For 50MB ~3125 sec.
  // Instead, allocate based on fileSize / 10000 ~ sec, cap 10000 sec (2.7h)
  uint32_t estSec = m_audioFileSize / 10000 + 10;
  if(estSec > 10000) estSec = 10000;
  if(estSec < 10) estSec = 10;

  size_t need = (estSec + 1) * sizeof(uint32_t);
  if(psramFound()){
    m_aacSecOff = (uint32_t*) ps_malloc(need);
  } else {
...
void Audio::omnia_aacIndexTick(uint16_t maxFrames){
  ...
  for(uint16_t i=0;i<maxFrames;i++){
    ...
    // Valid ADTS frame
    if(m_aacSampleRate==0) m_aacSampleRate = sr;
    m_aacBuildCumBytes += fl;
    m_aacBuildCumSamples += smp;
    m_aacBuildPos += fl;
...
bool Audio::omnia_aacSeekMs(uint32_t ms){
  if(m_codec != CODEC_AAC) return false;
  if(m_audioDataSize==0 && m_audioFileSize==0) return false;
  if(m_aacDurMs==0 && m_audioFileDuration>0) m_aacDurMs = m_audioFileDuration*1000UL;
  if(m_aacDurMs==0){
    uint32_t br = m_nominal_bitrate ? m_nominal_bitrate : m_avr_bitrate;
    if(br==0) br=128000;
    if(m_audioFileSize) m_aacDurMs = (uint64_t)m_audioFileSize*8*1000 / br;
  }
  if(m_aacDurMs==0) return false;
  if(ms > m_aacDurMs) ms = m_aacDurMs;
