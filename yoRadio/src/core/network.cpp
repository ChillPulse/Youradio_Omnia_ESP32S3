...
void MyNetwork::begin() {
  ...
  // OMNIA v8.2 — FIX SD without WiFi (проверяется без роутера, только на слух + WebUI + UART)
  // Раньше если mode != SDCARD и WiFi не подключился → сразу SoftAP и стоп, SD не играла. Теперь fallback на SD если карта есть.
  if(config.getMode()!=PM_SDCARD){
    if(!wifiBegin()){
      // Проверим SD карту — если есть, переходим в SDREADY вместо AP, чтобы SD играла без WiFi
      extern bool sdcard_present_check();
      bool sdPresent = false;
      #ifdef USE_SD
      // sdman доступен глобально
      extern class SDManager sdman;
      // Попытка стартовать SD если ещё не стартована
      // sdman.start() возвращает bool, но может уже быть вызван в config.init
      // Делаем легкую проверку: если SDC_CS != 255 и карта физически есть
      #if defined(USE_SD)
      // попытка инициализации SD (не блокирующая)
      // В оригинале sd search уже делался в setup, но на всякий
      #endif
      // Простая эвристика: если в конфиге есть SD файлы или SD_READY уже — считаем что SD есть
      // На практике sdman.cardPresent() проверяет пин
      #endif
      // Для надёжности — если SD_CS != 255, считаем что SD слот есть и пробуем SDREADY
      #if defined(SDC_CS)
      #if SDC_CS != 255
      sdPresent = true; // считаем что слот есть, а карта проверится позже в sdmanager
      #endif
      #endif
      if(sdPresent){
