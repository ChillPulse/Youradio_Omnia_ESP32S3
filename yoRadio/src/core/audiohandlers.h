...
void audio_info(const char *info) {
  ...
  if (((ici = strstr(info, "StreamTitle= ")) != NULL) /*&& strlen(info) > 15*/) {
    if(strlen(config.station.title)==0 || config.station.title==config.station.name || strstr(config.station.title, "timeout") != NULL){
...
bool printable(const char *info) {
  ...
  for (uint8_t c = 0; c < strlen(info); c++) {
    ...
    if (!p) p = (uint8_t)info[c] >= 0xC2 && (uint8_t)info[c+1] >= 0x80 && (uint8_t)info[c+1] <= 0xBF;
  }
  return true;
}
