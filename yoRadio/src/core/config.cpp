...
//          if(_bootDone) Serial.println("SD Mounted"); else BOOTLOG("SD Mounted");
          ...
          if(_bootDone) Serial.print("Waiting for SD card indexing...\t"); else Serial.print("##[BOOT]#\tWaiting for SD card indexing...\t\t");
          initSDPlaylist();
...
void Config::setDefaults() {
  ...
  strlcpy(store.sntp2,"2.ru.pool.ntp.org", 35);
...
//        BOOTLOG("audio:\t\t%s (CS-%d, DCS-%d, DREQ-%d, RST-%d, SPI-%s, MOSI-%d, MISO-%d, SCK-%d)", "VS1053", VS1053_CS, VS1053_DCS, VS1053_DREQ, VS1053_RST, VS_HSPI?"HSPI":"VSPI", mosi_pin, miso_pin, sclk_pin);
  ...
  BOOTLOG("audioinfo:\t%s", config.store.audioinfo?"true":"false");
  BOOTLOG("smartstart:\t%d", config.store.smartstart);
  BOOTLOG("vumeter:\t%s", config.store.vumeter?"true":"false");
  BOOTLOG("softapdelay:\t%d", config.store.softapdelay);
  BOOTLOG("flipscreen:\t%s", config.store.flipscreen?"true":"false");
  BOOTLOG("invertdisplay:\t%s", config.store.invertdisplay?"true":"false");
  BOOTLOG("showweather:\t%s", config.store.showweather?"true":"false");
